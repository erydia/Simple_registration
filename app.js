...
app.post('/register', (req, res) => {
  ...
  const requestedLogin = registeredUsers.find(el => el.userLogin === login);

  if (login !== '' && !requestedLogin) {
...
app.post('/login', (req, res) => {
  ...
  const registeredUser = registeredUsers.find(el => el.userLogin === login);
  if (typeof registeredUser !== 'undefined') {
    if (registeredUser.userPassword === password) {
